class FloatOperatorEB extends ExpressionBlock{
    /**
    ...
    GetEvalType(){
        return new ExpressionTypeConstr([], ConstrType.Get("float"));
    }

    Duplicate(){
        let copy = new FloatOperatorEB(this.symbol);
    ...
    CheckValid(){
        if(this.childrenBlocks[0] != null){
            ...
            let [typeValid, _1, _2] = this.childrenBlocks[0].GetEvalType().CheckCompatibilityWith(new ExpressionTypeConstr([], ConstrType.Get("float")));
        ...
        if(this.childrenBlocks[1] != null){
            ...
            let [typeValid, _1, _2] = this.childrenBlocks[1].GetEvalType().CheckCompatibilityWith(new ExpressionTypeConstr([], ConstrType.Get("float")));
            if(!typeValid){return false;} // invalid type
        }
        return true;
    }
