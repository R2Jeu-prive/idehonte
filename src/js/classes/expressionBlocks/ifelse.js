class IfElseEB extends ExpressionBlock {
    constructor() {
        super(
            ExpressionBlock.text("if") +
            ExpressionBlock.emptySlot +
            ExpressionBlock.text("then") +
            ExpressionBlock.emptySlot +
            ExpressionBlock.text("else") +
            ExpressionBlock.emptySlot
        );
    ...
    GetEvalType() {
        const type1 = this.childrenBlocks[1].GetEvalType();
        const type2 = this.childrenBlocks[2].GetEvalType().DistinguishIdentsWith(type1);
        const [typeValid, finalType, _2] = type1.CheckCompatibilityWith(type2);
        if (!typeValid) {
            console.error("GetEvalType on if else where both case are incompatible (this should never happen)");
        }
        return finalType;
    }
    ...
    CheckValid() {
        if (this.childrenBlocks[0] != null) {
            ...
            let [typeValid, _1, _2] = this.childrenBlocks[0]
                .GetEvalType()
                .CheckCompatibilityWith(new ExpressionTypeConstr([], ConstrType.Get("bool")));
            if (!typeValid) {
                return false;
            } // invalid type
        }
        ...
        if (this.childrenBlocks[2] != null) {
            if (!(this.childrenBlocks[2] instanceof ExpressionBlock)) {
                return false;
            } // not an expression
            if (!this.childrenBlocks[2].CheckValid()) {
                return false;
            } // children not valid
        }
