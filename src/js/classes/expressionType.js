...
function GetNewIdent(usedIdents) {
    //[TODO] check this looks very weird ???
    let start = "";
    while (true) {
        for (let i = 0; i < 26; i++) {
...
class ExpressionType {
    ...
    GetIdentChars() {
        if (this instanceof ExpressionTypeIdent) {
            ...
            this instanceof ExpressionTypeArrow ||
            this instanceof ExpressionTypeStar ||
            this instanceof ExpressionTypeConstr
        ) {
            let res = [];
    ...
    ReplaceIdent(searchIdent, expressionType) {
        if (this instanceof ExpressionTypeIdent) {
            ...
            copy.childrenExpressionTypes = copy.childrenExpressionTypes.map((el) =>
                el.ReplaceIdent(searchIdent, expressionType)
            );
            return copy;
        }
    }
        ...
        if (otherExpressionType instanceof ExpressionTypeConstr) {
            if (this instanceof ExpressionTypeConstr) {
                if (this.childConstrType.name != otherExpressionType.childConstrType.name) {
                    ...
                    let otherStar = new ExpressionTypeStar(
                        otherExpressionType.childrenExpressionTypes.map((el) => el.DeepCopy())
                    );
                    let [valid, resultingExpressionType, identReplacements] =
                        thisStar.CheckCompatibilityWith(otherStar);
                    if (!valid) {
                        return [false, null, null]; // exemple (int*float) list != (float*int) list
                    } else {
                        return [
                            true,
                            new ExpressionTypeConstr(
                                resultingExpressionType.childrenExpressionTypes.map((el) => el.DeepCopy()),
                                this.childConstrType
                            ),
                            identReplacements,
                        ]; // exemple ('a, 'b) graph = ('c, 'c) graph
                    }
                }
            } else {
        ...
        if (otherExpressionType instanceof ExpressionTypeStar) {
            if (this instanceof ExpressionTypeStar) {
                if (this.childrenExpressionTypes.length != this.childrenExpressionTypes.length) {
                    ...
                    let globalResultingExpressionType = new ExpressionTypeStar(
                        new Array(thisCopy.childrenExpressionTypes.length)
                    );
                    let globalIdentReplacements = [];
                    for (let i = 0; i < thisCopy.childrenExpressionTypes.length; i++) {
                        let [valid, resultingExpressionType, identReplacements] = thisCopy.childrenExpressionTypes[
                            i
                        ].CheckCompatibilityWith(otherCopy.childrenExpressionTypes[i]);
                        if (!valid) {
                            ...
                            for (let r = 0; r < identReplacements.length; r++) {
                                ...
                                for (let j = 0; j < globalIdentReplacements; j++) {
                                    globalIdentReplacements[j][1] = globalIdentReplacements[j][1].ReplaceIdent(
                                        identReplacements[r][0],
                                        identReplacements[r][1]
                                    );
                                }
                                for (let j = 0; j < i; j++) {
                                    globalResultingExpressionType.childrenExpressionTypes[j] =
                                        globalResultingExpressionType.childrenExpressionTypes[j].ReplaceIdent(
                                            identReplacements[r][0],
                                            identReplacements[r][1]
                                        );
                                }
        ...
        if (otherExpressionType instanceof ExpressionTypeArrow) {
            if (this instanceof ExpressionTypeArrow) {
                if (this.childrenExpressionTypes.length != this.childrenExpressionTypes.length) {
                    ...
                    let thisCopy = this.DeepCopy();
                    let otherCopy = otherExpressionType.DeepCopy();
                    let globalResultingExpressionType = new ExpressionTypeArrow(
                        new Array(thisCopy.childrenExpressionTypes.length)
                    );
                    let globalIdentReplacements = [];
                    for (let i = 0; i < thisCopy.childrenExpressionTypes.length; i++) {
...
class ExpressionTypeIdent extends ExpressionType {
    //ex: 'a
...
class ExpressionTypeArrow extends ExpressionType {
    //ex: int -> float -> string
    ...
    constructor(childrenExpressionTypes_) {
        ...
        if (childrenExpressionTypes_[childrenExpressionTypes_.length - 1] instanceof ExpressionTypeArrow) {
            ...
            length =
                length +
                childrenExpressionTypes_[childrenExpressionTypes_.length - 1].childrenExpressionTypes.length -
                1;
        }
        ...
        if (childrenExpressionTypes_[childrenExpressionTypes_.length - 1] instanceof ExpressionTypeArrow) {
            for (let i = childrenExpressionTypes_.length - 1; i < length; i++) {
                this.childrenExpressionTypes[i] =
                    childrenExpressionTypes_[childrenExpressionTypes_.length - 1].childrenExpressionTypes[
                        i - childrenExpressionTypes_.length + 1
                    ];
            }
        } else {
...
class ExpressionTypeStar extends ExpressionType {
    //ex: int * float * string
    /** @param {ExpressionType[]} childrenExpressionTypes_*/
    constructor(childrenExpressionTypes_) {
        super();
...
class ExpressionTypeConstr extends ExpressionType {
    //ex: ('a, 'b) graph
    /**
     * @param {ExpressionType[]} childrenExpressionTypes_
    ...
    DeepCopy() {
        return new ExpressionTypeConstr(
            this.childrenExpressionTypes.map((child) => child.DeepCopy()),
            this.childConstrType
        );
    }
}
