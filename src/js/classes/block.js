...
class Block {
    ...
    constructor(text_) {
        this.id = Date.now();
        while (Block.all[this.id] != undefined) {
            this.id++;
        }
    ...
    SetShopCategory(category) {
        if (!this.isInShop) {
            console.error("can't assign shop category to a block that is not in shop");
            return;
        }
